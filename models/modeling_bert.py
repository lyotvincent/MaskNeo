...
class BertForTokenAttentionSparseCLSJoint(BertPreTrainedModel):
    ...
    def forward(
        ...
        #print(outputs["last_hidden_state"].shape, self.learnable_mask.shape)
        bsz, seq_len, hidden_size = outputs["last_hidden_state"].shape

        masked_embeddings, mask_weight = self.learnable_mask(outputs["last_hidden_state"])
...
class BertForTokenAttentionSparseCLSJoint_incremental(BertPreTrainedModel):
    base_model_prefix = "model"

    def __init__(self, config: BertConfig, training_mask_layer):
        '''

        :param config:
        :param training_mask_layer: range [1, 24]
        '''
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
    ...
    def forward(
        ...
        for cur_layer in range(hidden_state_layer, 25):
            ...
            if cur_layer < 24:
                next_hidden_state = \
                self.bert.encoder.layer[cur_layer](cur_layer_masked_embeddings, attention_mask, head_mask)[0]
            else:
...
class BertForTokenAttentionSparseCLSJointMultiClass(BertPreTrainedModel):
    def __init__(self, config: BertConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        encoder_outputs = backbone_model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
